...
eplControllers.controller('HomeCtrl',['$scope','$location','$auth','$rootScope', '$state', '$http', '$mdDialog', 'FectchData',
	function($scope,$location,$auth,$rootScope, $state, $http, $mdDialog, FectchData){
	if(!$auth.isAuthenticated()){
		$location.path('/login');
	}
	$rootScope.dataCenter = {
		users: {},
		rooms: {}
	};
	var payload = $auth.getPayload();
	
	FectchData.getUsers($scope.dataCenter.users);
	FectchData.getJoinedRooms($scope.dataCenter.rooms);
	$scope.effectLevel = 10;
	...
	$scope.showUsers = function(ev){
		$mdDialog.show({
			controller: ['$scope', '$mdDialog', function($scope, $mdDialog){
				$scope.cancel = function(){
					$mdDialog.cancel();
				};
				$scope.users = $rootScope.dataCenter.users;
			}],
			templateUrl: 'templates/usersDialog.tmpl.html',
